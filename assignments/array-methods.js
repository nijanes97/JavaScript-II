...
let fullNames = [];
runners.forEach(function(current_value){
  fullNames.push(current_value.first_name + " " + current_value.last_name);
});
...
firstNamesAllCaps = runners.map(a => a.first_name.toUpperCase());
console.log(firstNamesAllCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue. We need a filtered version of the runners array, containing only those runners with large sized shirts so they can choose a different size. This will be an array of objects.
let runnersLargeSizeShirt = [];
runnersLargeSizeShirt.push(runners.filter(function(current_value) {
  if(current_value.shirt_size === "L"){
    return true;
  }
}));
...
runners.reduce(function(sum, current_value){
  ticketPriceTotal += current_value.donation;
});
...
// Problem 1
// thanking the highest donation
let runnerHighestDono = runners[0];
runners.forEach(function(current_value){
  if(current_value.donation > runnerHighestDono.donation){
    runnerHighestDono = current_value;
  }
});
console.log("Thank you, " + fullNames[runnerHighestDono.id - 1] + ", for your generous donation of $" + runnerHighestDono.donation + "!");

// Problem 2
// if you need to email all the runners info about the race
let emailList = [];
emailList.push(runners.map(current_value => current_value.email));
console.log(emailList);

// Problem 3
// make an ordered list of all shirt sizes for the runners
let runnerShirtSizes = [];
runnerShirtSizes.push(runners.filter(function(current_value){
  if(current_value.shirt_size === "XS"){
    return true;
  }
}));
runnerShirtSizes.push(runners.filter(function(current_value){
  if(current_value.shirt_size === "S"){
    return true;
  }
}));
runnerShirtSizes.push(runners.filter(function(current_value){
  if(current_value.shirt_size === "M"){
    return true;
  }
}));
runnerShirtSizes.push(runners.filter(function(current_value){
...
runnerShirtSizes.push(runners.filter(function(current_value){
  if(current_value.shirt_size === "XL"){
    return true;
  }
}));
runnerShirtSizes.push(runners.filter(function(current_value){
  if(current_value.shirt_size === "2XL"){
    return true;
  }
}));
runnerShirtSizes.push(runners.filter(function(current_value){
  if(current_value.shirt_size === "3XL"){
    return true;
  }
}));

console.log(runnerShirtSizes);